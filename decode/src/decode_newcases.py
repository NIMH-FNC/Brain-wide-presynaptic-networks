...
from util import get_mapping, get_session, load_yml, normalize, get_session_newcase
...
def get_data(data):
    ...
    if "pair" in data:
        ...
        fSpks1, fSpeed1, fWhisk1, fStim1, fSound1, iscell1, spontaneous1, _, tnames1 = (
            get_session(data["day1"])
        )
        fSpks2, fSpeed2, fWhisk2, fStim2, fSound2, iscell2, spontaneous2, _, tnames2 = (
            get_session(data["day2"])
        )
        ...
        # neuromod
        path = data["path"]
        pre, mouse, *_ = path.split("_")
        pair = path.split("/")[-1]
        day1 = "pre"
        day2 = "post"
        mouse = mouse.removeprefix("Mouse")
        # print(mouse, day1, day2)
        fSpks, fSpeed, fWhisk, fStim, fSound, iscell, spontaneous, drug, tnames = (
            get_session_newcase(data)
        )

        # Baseline
        fSpks0 = np.row_stack(
            [t for t, s, d in zip(fSpks, tnames, drug) if d == 0]
        )
        fSpeed0 = np.concatenate(
            [t for t, s, d in zip(fSpeed, tnames, drug) if d == 0]
        )
        fWhisk0 = np.concatenate(
            [t for t, s, d in zip(fWhisk, tnames, drug) if d == 0]
        )
        ...
        # Drug 1
        fSpks1 = np.row_stack(
            [t for t, s, d in zip(fSpks, tnames, drug) if d == 1]
        )
        fSpeed1 = np.concatenate(
            [t for t, s, d in zip(fSpeed, tnames, drug) if d == 1]
        )
        fWhisk1 = np.concatenate(
            [t for t, s, d in zip(fWhisk, tnames, drug) if d == 1]
        )
        ...
        if 2 in drug:
            fSpks2 = np.row_stack(
                [t for t, s, d in zip(fSpks, tnames, drug) if d == 2]
            )
            fSpeed2 = np.concatenate(
                [t for t, s, d in zip(fSpeed, tnames, drug) if d == 2]
            )
            fWhisk2 = np.concatenate(
                [t for t, s, d in zip(fWhisk, tnames, drug) if d == 2]
            )
    ...
    file_path = Path.cwd() / f'output/{pair}_trials.csv'
    file_path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame({'trial': tnames, 'drug': drug}).to_csv(file_path)
    
    if DEBUG:
        df_data = pd.DataFrame(X0)
        df_data['speed'] = fSpeed0
        df_data['whisk'] = fWhisk0
        df_data.to_csv(f'output/{pair}_0.csv')

        df_data = pd.DataFrame(X1)
        df_data['speed'] = fSpeed1
        df_data['whisk'] = fWhisk1        
        df_data.to_csv(f'output/{pair}_1.csv')

        if X2 is not None:
            df_data = pd.DataFrame(X2)
            df_data['speed'] = fSpeed2
            df_data['whisk'] = fWhisk2        
            df_data.to_csv(f'output/{pair}_2.csv')
...
def decode_cross(data):
    ...
    for dv in ["speed", "whisk"]:
        ...
        (
            ...
            rsq_1_test_cell,
            ridge_all,
        ) = decoder(X1, y1, C1, alphas=alphas, cv=cv, fit_intercept=False)
        ys = []
        yhats = []
        ...
        savemat(
            f"outputs/{pair}_Day1_{dv}_decode.mat",
            {
                "true_trace": ys,
                "predicted_trace": yhats,
                "weight": ridge_all.coef_,
                "trials": data1["tnames"],
            },
        )
        ...
        (
            ...
            rsq_2_test_cell,
            ridge_all,
        ) = decoder(X2, y2, C2, alphas=alphas, cv=cv, fit_intercept=False)
        ys = []
        yhats = []
        ...
        yhats = np.column_stack(yhats)
        savemat(
            f"outputs/{pair}_Day2_{dv}_decode.mat",
            {
                "true_trace": ys,
                "predicted_trace": yhats,
                "weight": ridge_all.coef_,
...
def decode_drug(data, dry=False):
    # df = pd.DataFrame(
    #     columns=[
    #         "mouse",
    #         "session",
    ...
    for dv in ["speed", "whisk"]:
        ...
        # ridge2, rsq_2_training, rsq_2_test, rsq_2_training_cell, rsq_2_test_cell, ridge_all

        ridge0, rsq_0_training, rsq_0_test, _, _, _ = decoder(
            X0, y0, None, alphas=alphas, cv=cv, fit_intercept=True
        )
        ridge1, rsq_1_training, rsq_1_test, _, _, _ = decoder(
            X1, y1, None, alphas=alphas, cv=cv, fit_intercept=True
        )
        ...
        if C2 is not None:
            ...
            ridge2, rsq_2_training, rsq_2_test, _, _, _ = decoder(
                X2, y2, None, alphas=alphas, cv=cv, fit_intercept=True
            )

            # df.loc[-1] = (mouse, pair, dv, "drug2", rsq_2_training, ridge2.score(X0, y0), ridge2.score(X1, y1), rsq_2_test)
            df.append(
                {
                    "mouse": mouse,
                    "session": pair,
...
def plot(name, df):
    ...
    for k, behavior in enumerate(["speed", "whisk"]):
        ...
        ax.scatter(x, y)
        ax.axline((0, 0), slope=1.0, ls="--", color="k")
...
if __name__ == "__main__":
    dataset = load_yml("data/Neuromod/neuromod_newcases.yml")
    dfs = []
    # r2s = []
    for d in dataset:
        df, r2 = decode_drug(d, DRY)
        dfs.append(df)
        # r2s.append(r2)
    if not DRY:
        merged_df = pd.concat(dfs, ignore_index=True)
        merged_df.to_csv("output/neuromod_newcases_decode_bnorm.csv", index=False)
