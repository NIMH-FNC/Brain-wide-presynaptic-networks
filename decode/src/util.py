...
def get_session(sess, window=15, check_nan=True):
    ...
    try:
        ...
        # matfile v7.3
        data = pymatreader.read_mat(path)
...
def get_session_newcase(sess, window=15, check_nan=True):
    ...
    group_path = f"{path[:-9]}_groups.mat"
    # print(group_path)
    group_mat = pymatreader.read_mat(group_path)
    spont = group_mat['groups']['spont']  # baseline
    spontLed = group_mat['groups']['spontLed']  # drug 1
    spontLedCl = group_mat['groups'].get('spontLedCl', [])  # drug 2
    ...
    for trial, tname, mask, led, ledcl in zip(trials, trial_names, caTrialsMask, caTrialsMaskLED, caTrialsMaskLEDCL):
        ...
        if tname in spont:
            drug.append(0)
        elif tname in spontLed:
            drug.append(1)
        elif tname in spontLedCl:
            drug.append(2)
        else:
            drug.append(np.nan)

    return lSpks, lSpeed, lWhisk1, lStim, lSound, iscell, spont, drug, tnames
